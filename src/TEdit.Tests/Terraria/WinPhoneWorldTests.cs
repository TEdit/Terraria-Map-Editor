...
public class WinPhoneWorldTests
{
    ...
    [Fact]
    public async Task SaveAsWinPhone_DesktopWorld_AppliesNativeCompatibilityLimits()
    {
        World world = new(40, 60, new string('T', 200))
        {
            Tiles = new Tile[60, 40],
        };
        world.Tiles[1, 1] = new Tile
        {
            IsActive = true,
            Type = 600,
            Wall = 300,
            LiquidAmount = 255,
            LiquidType = LiquidType.Shimmer,
        };
        Chest chest = new(2, 2) { MaxItems = 40 };
        chest.Items[0] = new Item(50_000, 1, 0);
        world.Chests.Add(chest);
        world.NPCs.Add(new NPC { SpriteId = 300 });

        string savedPath = Path.Combine(Path.GetTempPath(), $"tedit-winphone-converted-{Guid.NewGuid():N}.world");
        try
        {
            await World.SaveWinPhoneAsync(world, savedPath);
            var (reloaded, error) = World.LoadWorld(savedPath);

            Assert.Null(error);
            Assert.True(world.IsWinPhone);
            Assert.Equal((uint)60, world.Version);
            Assert.True(reloaded.IsWinPhone);
            Assert.Equal((uint)60, reloaded.Version);
            Assert.Equal(128, reloaded.Title.Length);
            Assert.False(reloaded.Tiles[1, 1].IsActive);
            Assert.Equal(0, reloaded.Tiles[1, 1].Wall);
            Assert.Equal(0, reloaded.Tiles[1, 1].LiquidAmount);
            Assert.Equal(short.MaxValue, reloaded.Chests[0].Items[0].StackSize);
            Assert.Empty(reloaded.NPCs);
            Assert.True(World.ValidateWorldFile(savedPath).IsValid);
    ...
    [Fact]
    public void SaveAsWinPhone_ExpandedWorldMaximum_IsEnforced()
    {
        World tooWide = new(1, World.WinPhoneMaxWorldWidth + 1, "Too wide");
        using MemoryStream output = new();

        NotSupportedException error = Assert.Throws<NotSupportedException>(() =>
            World.SaveWinPhoneConverted(tooWide, output));

        Assert.Contains("4200 × 1200", error.Message);
    }
