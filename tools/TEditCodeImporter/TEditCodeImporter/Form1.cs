...
namespace TEditCodeImporter
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            AllocConsole();
            InitializeComponent();
            terraria = TerrariaWrapper.LaunchGame();
        }
        ...
        {
            var loc = Language.GetText(name);
            return loc.Value;

            name = name.Replace("ItemName.", "");
            name = name.Replace("NPCName.", "");
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                outputText += string.Format("{0}: {1}\r\n", i, FixupName(terraria.Recipes[i].createItem.Name));

            }
            for (int i = 0; i < 1000; i++)
            {
                var item = terraria.GetItem(i);
                outputText += string.Format("{0}: {1}\r\n", i, FixupName(item.Name));
            }
