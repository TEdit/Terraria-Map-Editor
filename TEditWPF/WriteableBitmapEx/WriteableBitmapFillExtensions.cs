...
namespace System.Windows.Media.Imaging
{
   ...
   public static partial class WriteableBitmapExtensions
   {
...
#else
                  NativeMethods.CopyUnmanagedMemory((IntPtr) pixels, srcOffsetBytes, (IntPtr) pixels, y * SizeOfArgb, len);
#endif
              }
#if !SILVERLIGHT
          }
              ...
              for (int y = yMin; y <= yMax; y++)
              {
                  ...
                  for (int i = 2; i < pn; i += 2)
                  {
                       ...
                       || vyj < y && vyi >= y)
                      {
                          ...
                          intersectionsX[intersectionCount++] = (int)(vxi + (y - vyi) / (float)(vyj - vyi) * (vxj - vxi));
                      }
                      vxi = vxj;
                      vyi = vyj;
                  }
      ...
      #region Beziér
       ...
       private static System.Collections.Generic.List<int> ComputeBezierPoints(int x1, int y1, int cx1, int cy1, int cx2, int cy2, int x2, int y2)
      {
      ...
      {
         // First segment
         var list = ComputeSegmentPoints(points[0], points[1], points[0], points[1], points[2], points[3], points[4], points[5], tension);

         // Middle segments
         int i; 
         for (i = 2; i < points.Length - 4; i += 2)
         {
            list.AddRange(ComputeSegmentPoints(points[i - 2], points[i - 1], points[i], points[i + 1], points[i + 2], points[i + 3], points[i + 4], points[i + 5], tension));
         }

         // Last segment
         list.AddRange(ComputeSegmentPoints(points[i - 2], points[i - 1], points[i], points[i + 1], points[i + 2], points[i + 3], points[i + 2], points[i + 3], tension));
      ...
      public static void FillCurveClosed(this WriteableBitmap bmp, int[] points, float tension, int color)
      {
         ...
         // First segment
         var list = ComputeSegmentPoints(points[pn - 2], points[pn - 1], points[0], points[1], points[2], points[3], points[4], points[5], tension);
         ...
         // Last segment
         list.AddRange(ComputeSegmentPoints(points[i - 2], points[i - 1], points[i], points[i + 1], points[i + 2], points[i + 3], points[0], points[1], tension));

         // Last-to-First segment
         list.AddRange(ComputeSegmentPoints(points[i], points[i + 1], points[i + 2], points[i + 3], points[0], points[1], points[2], points[3], tension));
