...
namespace System.Windows.Media.Imaging
{
   ...
   public static partial class WriteableBitmapExtensions
   {
...
#else
      private static void DrawCurveSegment(int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, float tension, int color, IntPtr pixels, int w, int h)
#endif
      {
         // Determine distances between controls points (bounding rect) to find the optimal stepsize
...
#else
         bmp.Lock();
         var pixels = bmp.BackBuffer;
#endif

         // First segment
         DrawCurveSegment(points[0], points[1], points[0], points[1], points[2], points[3], points[4], points[5], tension, color, pixels, w, h);

         // Middle segments
         int i;
         for (i = 2; i < points.Length - 4; i += 2)
         {
            DrawCurveSegment(points[i - 2], points[i - 1], points[i], points[i + 1], points[i + 2], points[i + 3], points[i + 4], points[i + 5], tension, color, pixels, w, h);
         }

         // Last segment
         DrawCurveSegment(points[i - 2], points[i - 1], points[i], points[i + 1], points[i + 2], points[i + 3], points[i + 2], points[i + 3], tension, color, pixels, w, h);
#if !SILVERLIGHT
      ...
      public static void DrawCurveClosed(this WriteableBitmap bmp, int[] points, float tension, int color)
      {
         ...
         // First segment
         DrawCurveSegment(points[pn - 2], points[pn - 1], points[0], points[1], points[2], points[3], points[4], points[5], tension, color, pixels, w, h);
         ...
         // Last segment
         DrawCurveSegment(points[i - 2], points[i - 1], points[i], points[i + 1], points[i + 2], points[i + 3], points[0], points[1], tension, color, pixels, w, h);
         
         // Last-to-First segment
         DrawCurveSegment(points[i], points[i + 1], points[i + 2], points[i + 3], points[0], points[1], points[2], points[3], tension, color, pixels, w, h);
#if !SILVERLIGHT
         bmp.AddDirtyRect(new Int32Rect(0,0,w,h));
         bmp.Unlock();
#endif
